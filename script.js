...
// добавление раздела, где будет храниться контент
const contents = document.createElement('div');
contents.className += ' contents';
document.body.appendChild(contents);

// добавление основного flex-контейнера для карточек
const catalog = document.createElement('div');
catalog.className += ' catalog';
contents.appendChild(catalog);

for (var i = 0; i < cats.length; i++) {

    // добавление контейнера карточки
    let item = document.createElement('div');
    item.className += " item";
    item.id = String(cats[i]["id"]);
    catalog.appendChild(item);

    // добавление визуальной части карточки (flex-контейнер)
    let visual = document.createElement('div');
    visual.className += " visual";
    visual.style.backgroundImage = "url(" + cats[i]["img_link"] + ")";
    ...
    if (cats[i]["favourite"]) { catName.className += "catNameFav"; } else { catName.className += "catName"; }

    let age = document.createElement('div');
    age.className += "age";
    let years = cats[i]["age"] == 1 ? " год" : cats[i]["age"] < 5 ? " года" : " лет";
    age.innerHTML = cats[i]["age"] + years;

    let rating = document.createElement('div');
    rating.className += "rating";
    rating.innerHTML = "&#x2605".repeat(cats[i]["rate"]) + "&#x2606".repeat(10 - cats[i]["rate"]);

    visual.append(catName, age, rating)

    // добавление описания в каточку
    let description = document.createElement('div');
    description.className += " description";
    description.innerHTML = cats[i]["description"];
